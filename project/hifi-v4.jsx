...
function Satellites({ epoch, hoveredFace, onHover }) {
  // 6 satellites, 6 faces, 1:1. Order around the cube follows the rotation
  // map so the satellite roughly sits in the direction its face points to.
  //   front  → Начало        (вход в продукт)
  //   top    → Твоя секунда  (взгляд сверху на масштаб)
  //   right  → Линия жизни   (время идёт вправо)
  //   left   → Современники  (контекст слева)
  //   back   → Эпохи         (обратная перспектива времени)
  //   bottom → Изобретения   (фундамент, на чём всё стоит)
  const sats = [
    { ang: -100, label: 'Начало',       sub: 'ввод даты',      face: 'front',  e: 'contemporary' },
    { ang: -160, label: 'Твоя секунда', sub: '24ч = 5000 лет', face: 'top',    e: 'ancient'      },
    { ang:  -40, label: 'Современники', sub: 'кто был жив',    face: 'left',   e: 'renaissance'  },
    { ang:   20, label: 'Линия жизни',  sub: 'две шкалы',      face: 'right',  e: epoch          },
    { ang:  100, label: 'Эпохи',        sub: 'scroll-лента',   face: 'back',   e: 'medieval'     },
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 560, position: 'relative' }}>
          ...
          <div style={{
            ...
            <button
              onClick={() => setTweak('assembled', !t.assembled)}
              style={{
                padding: '14px 28px',
                background: accent,
                color: '#fff',
            ...
            >{t.assembled ? '→ разобрать кубик' : 'собрать кубик →'}</button>
          </div>
