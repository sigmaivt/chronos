    ...
    <div style={{
      ...
      <div style={{
        ...
        {FACES.map((f) => {
          ...
          // Per-face: ready → repaint all 9 cells in user's epoch colour;
          // not ready → keep that face's mixed palette (looks scattered).
          const colors = isReady ? baseColors.map(() => accent) : baseColors;
          return (
            <CubeFace
              key={f.name}
              size={size}
...
function Satellites({ epoch, hoveredFace, onHover, readyFaces, onToggleReady }) {
  // 6 satellites, 6 faces, 1:1.
  //   front  → Ты            (вход: дата, место, личные вехи, длина жизни)
  //   top    → Идеи          (открытия, теории, законы)
  //   right  → Современники  (кто жил параллельно)
  //   left   → Искусство     (картины, музыка, литература)
  //   back   → Места          (постройки, до которых можно дотронуться)
  //   bottom → Изобретения   (вещи, технологии)
  const accent = EPOCHS[epoch].colors[0];
  const sats = [
  ...
  return (
    <>
      {sats.map((s, i) => {
        ...
        //   ready          → solid accent border + filled badge (DONE)
        //   not ready      → outlined, dimmed badge (TODO)
        //   hovered        → highlight on top of either
        const borderC = isReady ? accent : 'rgba(245,243,237,0.18)';
        const bgGrad = isReady
          ? `linear-gradient(135deg, ${accent}33 0%, rgba(15,15,25,0.78) 100%)`
          : 'rgba(15,15,25,0.55)';
        return (
          <div key={i}
            onMouseEnter={() => onHover && onHover(s.face)}
            onMouseLeave={() => onHover && onHover(null)}
            onClick={() => onToggleReady && onToggleReady(s.face)}
            style={{
              position: 'absolute',
              left: '50%', top: '50%',
              transform: `translate(calc(-50% + ${x}px), calc(-50% + ${y}px)) ${hovered ? 'scale(1.05)' : 'scale(1)'}`,
              opacity: hoveredFace && !hovered ? 0.45 : 1,
              transition: 'opacity 320ms ease, transform 320ms cubic-bezier(.3,.7,.3,1), box-shadow 320ms',
              width: 188,
              padding: '13px 15px',
              background: bgGrad,
              border: `1px solid ${hovered ? accent : borderC}`,
              backdropFilter: 'blur(14px)',
              WebkitBackdropFilter: 'blur(14px)',
              borderRadius: 14,
              boxShadow: hovered
                ? `0 12px 40px ${accent}44, inset 0 1px 0 rgba(255,255,255,0.06)`
                : isReady
                  ? `0 10px 32px ${accent}30, inset 0 1px 0 rgba(255,255,255,0.05)`
            ...
            <div style={{
              ...
              <div style={{
                ...
                <span style={{
                  ...
                  background: isReady ? accent : 'rgba(245,243,237,0.25)',
                  boxShadow: isReady ? `0 0 10px ${accent}` : 'none',
...
function HiFiHero({ t, setTweak, toggleReady }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Left column — title + readiness + CTA */}
        <div style={{ maxWidth: 560, position: 'relative' }}>
          ...
          <p style={{
            ...
            Открой грань, заполни — и она окрасится твоим цветом эпохи.
          </p>
          ...
          <div style={{
            ...
            <div style={{
              ...
              {readyCount === 0 && 'наведи курсор на грань — куб повернётся; клик — отметит готовой'}
              {readyCount > 0 && !allReady && `${totalFaces - readyCount} ${pluralFaces(totalFaces - readyCount)} ещё ждёт`}
              {allReady && 'все шесть граней собраны — можно разворачивать'}
            </div>
          </div>
          ...
          <div style={{
            ...
            <button
              onClick={() => hasReady && setTweak('unfolded', true)}
              disabled={!hasReady}
              style={{
                padding: '14px 30px',
