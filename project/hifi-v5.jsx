...
function CubeFace({ size, transform, colors, dim, highlighted, name, onEnter, onLeave, assembled, assembledColor }) {
  return (
    <div
      ...
      style={{
        ...
        boxShadow: highlighted
          ? `0 0 80px ${(assembled ? assembledColor : colors[4])}77, inset 0 0 0 1px rgba(255,255,255,0.12)`
...
function Cube3D({ size = 420, mouse, time, epoch, hoveredFace, onHover, assembled }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        {FACES.map((f) => {
          ...
          const colors = assembled
            ? baseColors.map(() => accent)
            : baseColors;
          return (
            <CubeFace
              key={f.name}
...
function Satellites({ epoch, hoveredFace, onHover }) {
  // 6 satellites, 6 faces, 1:1 (V5 mapping).
  ...
  const sats = [
    { ang: -100, label: 'Ты',           sub: 'дата · место · вехи',  face: 'front',  e: 'contemporary' },
    { ang: -160, label: 'Идеи',         sub: 'теории и открытия',    face: 'top',    e: 'ancient'      },
    { ang:  -40, label: 'Искусство',    sub: 'картины · музыка · текст', face: 'left',   e: 'renaissance'  },
    { ang:   20, label: 'Современники', sub: 'кто жил рядом',        face: 'right',  e: epoch          },
    { ang:  100, label: 'Места',        sub: 'до сих пор стоят',     face: 'back',   e: 'medieval'     },
    { ang:  160, label: 'Изобретения',  sub: 'что уже было',         face: 'bottom', e: 'modern'       },
  ];
  const R = 360;
...
function HiFiHero({ t, setTweak }) {
  ...
  if (t.unfolded) {
    return <UnfoldedTimeline epoch={t.epoch} onClose={() => setTweak('unfolded', false)} />;
  }
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 560, position: 'relative' }}>
          ...
          <div style={{
            ...
            <button
              onClick={() => {
                if (!t.assembled) setTweak('assembled', true);
                else setTweak('unfolded', true);
              }}
              style={{
                padding: '14px 28px',
                background: accent,
            ...
            >{t.assembled ? 'развернуть в линию →' : 'собрать кубик →'}</button>
            {t.assembled && (
              <button
                onClick={() => setTweak('assembled', false)}
                style={{
                  appearance: 'none', background: 'transparent', border: 'none',
                  padding: '14px 6px', cursor: 'pointer',
                  fontFamily: '"DM Sans", sans-serif', fontSize: 13,
                  color: HIFI_INK_DIM, textDecoration: 'underline',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <Satellites epoch={t.epoch} hoveredFace={hoveredFace} onHover={setHoveredFace} />
            <Cube3D
